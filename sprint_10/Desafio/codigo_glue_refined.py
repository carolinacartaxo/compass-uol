...
dim_movies_df = spark_df_local.join(
    ...
    when(col("belongs_to_collection").isNotNull(), col("belongs_to_collection.id")).alias("collection_id"),  # Abri belongs_to_collection e criei duas colunas
    when(col("belongs_to_collection").isNotNull(), col("belongs_to_collection.name")).alias("collection_name")
).dropDuplicates(["movie_id"]) # Para não repetir o mesmo filme várias vezes aqui por conta do join
